...
int test_vector_default_constructor() {
    ...
    if (!check((int)v.size(), 0, check_msg(get_type_name<T>(), "Default constructor should create empty vector"))) return TEST_FAIL;
...
int test_vector_size_constructor() {
    ...
    if (!check((int)v.size(), 10, check_msg(get_type_name<T>(), "Size constructor should set correct size"))) return TEST_FAIL;
...
int test_vector_size_value_constructor() {
    ...
    if (!check((int)v.size(), 5, check_msg(get_type_name<T>(), "Size+value constructor should set correct size"))) return TEST_FAIL;

    // Verify all elements are initialized
    for (int i = 0; i < 5; i++) {
        if (!check((double)v.data()[i], 3.5, check_msg(get_type_name<T>(), "Elements should be initialized with value"))) return TEST_FAIL;
    }
...
int test_vector_initializer_list_constructor() {
    ...
    if (!check((int)v.size(), 5, check_msg(get_type_name<T>(), "Initializer list constructor should set correct size"))) return TEST_FAIL;

    // Verify values
    T expected[] = {(T)1.0, (T)2.0, (T)3.0, (T)4.0, (T)5.0};
    for (int i = 0; i < 5; i++) {
        if (!check((double)v.data()[i], (double)expected[i], check_msg(get_type_name<T>(), "Elements should match initializer list"))) return TEST_FAIL;
    }
...
int test_vector_copy_constructor() {
    ...
    if (!check((int)v2.size(), (int)v1.size(), check_msg(get_type_name<T>(), "Copy constructor should copy size"))) return TEST_FAIL;

    // Verify data is copied, not referenced
    T expected[] = {(T)1.0, (T)2.0, (T)3.0};
    for (int i = 0; i < 3; i++) {
        if (!check((double)v2.data()[i], (double)expected[i], check_msg(get_type_name<T>(), "Copy constructor should copy data"))) return TEST_FAIL;
    }

    // Modify original - copy should not change
    v1.data()[0] = (T)99.0;
    if (!check((double)v2.data()[0], 1.0, check_msg(get_type_name<T>(), "Copy should be independent"))) return TEST_FAIL;
...
int test_vector_move_constructor() {
    ...
    if (!check((int)v2.size(), 3, check_msg(get_type_name<T>(), "Move constructor should transfer size"))) return TEST_FAIL;
...
int test_vector_size_attribute() {
    ...
    if (!check((int)v.size(), 25, check_msg(get_type_name<T>(), "Size attribute should return correct size"))) return TEST_FAIL;
...
int test_vector_data_access() {
    ...
    if (!check((double)v.data()[0], 5.0, check_msg(get_type_name<T>(), "Data access should work"))) return TEST_FAIL;

    v.data()[1] = (T)20.0;
    if (!check((double)v.data()[1], 20.0, check_msg(get_type_name<T>(), "Data modification should work"))) return TEST_FAIL;
...
int test_vector_const_data_access() {
    ...
    if (!check((double)data[0], 1.0, check_msg(get_type_name<T>(), "Const data access should work"))) return TEST_FAIL;
...
int test_vector_empty_check() {
    ...
    if (!check((int)empty.size(), 0, check_msg(get_type_name<T>(), "Empty vector should have size 0"))) return TEST_FAIL;

    if (!check((int)nonempty.size(), 5, check_msg(get_type_name<T>(), "Non-empty vector should have non-zero size"))) return TEST_FAIL;
...
int test_vector_copy_assignment() {
    ...
    if (!check((int)v2.size(), 3, check_msg(get_type_name<T>(), "Copy assignment should copy size"))) return TEST_FAIL;

    T expected[] = {(T)1.0, (T)2.0, (T)3.0};
    for (int i = 0; i < 3; i++) {
        if (!check((double)v2.data()[i], (double)expected[i], check_msg(get_type_name<T>(), "Copy assignment should copy data"))) return TEST_FAIL;
    }
    ...
    #endif
    if (!check((int)v1.size(), 3, check_msg(get_type_name<T>(), "Self-assignment should not break vector"))) return TEST_FAIL;
...
int test_vector_move_assignment() {
    ...
    if (!check((int)v2.size(), 3, check_msg(get_type_name<T>(), "Move assignment should transfer size"))) return TEST_FAIL;
...
int test_vector_memory_allocation() {
    ...
    // Verify some values
    if (!check((double)v.data()[0], 0.0, check_msg(get_type_name<T>(), "First element should be 0"))) return TEST_FAIL;

    if (!check((double)v.data()[999], 1498.5, check_msg(get_type_name<T>(), "Last element should be correct"))) return TEST_FAIL;
...
int test_vector_float_type() {
    ...
    if (!check((int)v.size(), 5, check_msg(get_type_name<float>(), "Float vector should have correct size"))) return TEST_FAIL;

    for (int i = 0; i < 5; i++) {
        if (!check(v.data()[i], 3.14f, check_msg(get_type_name<float>(), "Float vector should store float values"))) return TEST_FAIL;
    }
...
int test_vector_int_type() {
    ...
    if (!check((int)v.size(), 5, check_msg(get_type_name<int>(), "Int vector should have correct size"))) return TEST_FAIL;

    // Verify individual int values
    for (int i = 0; i < 5; i++) {
        if (!check(v.data()[i], i + 1, check_msg(get_type_name<int>(), "Int vector should store correct values"))) return TEST_FAIL;
    }
...
int test_vector_complex_type() {
    ...
    if (!check((int)v.size(), 3, check_msg(get_type_name<std::complex<double>>(), "Complex vector should have correct size"))) return TEST_FAIL;

    if (!check(v.data()[0], std::complex<double>(1.0, 2.0), check_msg(get_type_name<std::complex<double>>(), "Complex vector should store complex values"))) return TEST_FAIL;
