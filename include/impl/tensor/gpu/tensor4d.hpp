...
namespace lahva
{
    ...
    template<typename T>
    class Tensor4D_ : public virtual GPUTensor_<T>, public virtual cpu::Tensor4D_<T>
    {
        public:
            ...
            //! @return reference to Matrix element i,j,k,l
    ...
    class Tensor4D : public GPUTensor<T, Allocator, GPUAllocator>, public virtual Tensor4D_<T>
    {
    ...
    protected:
        ...
        inline size_t data_id_(size_t i, size_t j, size_t k, size_t l) const
        {
            ...
            assert(i < n_1_ && j < n_2_ && k < n_3_ && l < n_4_);
    ...
    public:
        Tensor4D() : GPUTensor<T, Allocator, GPUAllocator>{} {};
        //! construct a square Tensor4D with dimensions n x n x n x n
        //! It is not guaranteed that the values will be initialized
        Tensor4D(size_t n, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());
        //! construct a square n x n x n x n Tensor4D initialized with value val
        Tensor4D(size_t n, T val, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());

        //! construct a Tensor4D with dimensions shape.first x shape.second x shape.third x shape.fourth
        //! It is not guaranteed that the values will be initialized!
        Tensor4D(const Shape &shape, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());

        Tensor4D(const Shape &shape, std::initializer_list<T> init, bool row_major = false, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());

        template <typename U>
        Tensor4D(const Shape &shape, const CudaRuntime &cudart, const U &alloc)
            : Tensor4D<T,Allocator,GPUAllocator>(shape, cudart, static_cast<GPUAllocator>(alloc))
           {
            };

        Tensor4D(const Shape &shape, const CudaRuntime &cudart, const gpualloc_ptr &gpualloc = GPUAllocator());
        //! construct a Tensor4D of shape with initial values val
        Tensor4D(const Shape &shape, T val, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());
        ...
        //!                       Tensor4D object is destructed. Use with care!
        Tensor4D(const Shape &shape, T *data, bool take_ownership = true, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());
        Tensor4D(const Shape &shape, const T *data, const alloc_ptr &alloc = Allocator(), const gpualloc_ptr &gpualloc = GPUAllocator());
    ...
    void Tensor4D<T, Allocator, GPUAllocator>::check_size_(size_t n_1,
                                size_t n_2, size_t n_3, size_t n_4)
{
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    GPUTensor<T, Allocator, GPUAllocator>{shape.first*shape.second*shape.third*shape.fourth, alloc, gpualloc}, 
    n_1_{shape.first}, n_2_{shape.second}, n_3_{shape.third}, n_4_{shape.fourth}
    {
        this->check_size_(shape.first, shape.second, shape.third, shape.fourth);
    }

    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, std::initializer_list<T> init, bool row_major, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    Tensor4D(shape, alloc, gpualloc)
    {
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, const CudaRuntime &cudart, const gpualloc_ptr &gpualloc) : 
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, T *data, bool take_ownership, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    GPUTensor<T, Allocator, GPUAllocator>{alloc, gpualloc}, n_1_{shape.first}, n_2_{shape.second}, n_3_{shape.third}, n_4_{shape.fourth}
    {
        this->data_ = data;
        this->count_ = n_1_*n_2_*n_3_*n_4_;
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, T val, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    Tensor4D(shape, alloc, gpualloc)
    {
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(size_t n, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(Shape(n, n, n, n), alloc, gpualloc) 
    {}

    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(size_t n, T val, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(Shape(n, n, n, n), val, alloc, gpualloc) {}

    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Shape &shape, const T *data, const alloc_ptr& alloc , const gpualloc_ptr& gpualloc) : 
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(shape, alloc, gpualloc)
    {
        std::copy(data, data + this->data_size_(n_1_, n_2_, n_3_, n_4_), this->data_);
    };

    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::~Tensor4D()
    {
    ...
    // copy operations
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(const Tensor4D<T, Allocator, GPUAllocator> &other) : 
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator> &Tensor4D<T, Allocator, GPUAllocator>::operator=(const Tensor4D<T, Allocator, GPUAllocator> &other)
    {
        if (this != &other)
        {
    ...
    // move operations
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator>::Tensor4D(Tensor4D<T, Allocator, GPUAllocator> &&other) : 
    GPUTensor<T, Allocator, GPUAllocator>{other},
    n_1_{other.n_1_}, n_2_{other.n_2_}, n_3_{other.n_3_}, n_4_{other.n_4_}
    {
    ...
    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator> &Tensor4D<T, Allocator, GPUAllocator>::operator=(Tensor4D<T, Allocator, GPUAllocator> &&other)
    {
        if (this != &other)
        {
    ...
    template <typename T, class Allocator, class GPUAllocator>
    T &Tensor4D<T, Allocator, GPUAllocator>::operator()(size_t i, size_t j, size_t k, size_t l)
    {
        return this->data_[this->data_id_(i, j, k, l)];
    }

    template <typename T, class Allocator, class GPUAllocator>
    const T &Tensor4D<T, Allocator, GPUAllocator>::operator()(size_t i, size_t j, size_t k, size_t l) const
    {
        return this->data_[this->data_id_(i, j, k, l)];
    }

    template <typename T, class Allocator, class GPUAllocator>
    Tensor4D<T, Allocator, GPUAllocator> &Tensor4D<T, Allocator, GPUAllocator>::operator+=(T val)
    {
    #pragma omp for
    ...
    template <typename T, class Allocator, class GPUAllocator>
    void Tensor4D<T, Allocator, GPUAllocator>::print() const
    {
        Matrix<T> temp((n_1_*n_2_), (n_3_*n_4_)); 
        // Transform 4D tensor to 2D matrix with contracted indices for printing
        for (size_t mu = 0; mu < n_1_; mu++)
