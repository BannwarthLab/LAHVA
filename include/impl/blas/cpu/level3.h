...
namespace lahva
{
    namespace cpu
    {
        /// @brief General matrix-matrix product (double-precision) with explicit transpose flags.
        ///
        ...
        ///
        /// @param Ta   Pointer to a char indicating whether matrix A is transposed ("N"/"T").
        /// @param Tb   Pointer to a char indicating whether matrix B is transposed ("N"/"T").
        ...
        /// @brief General matrix-matrix product (double-precision) with defaults.
        ///
        /// Convenience overload that assumes no transposition by default and common default
        ...
        /// @param Ta    Transpose flag for A ("N" or "T"). Default: "N"
        /// @param Tb    Transpose flag for B ("N" or "T"). Default: "N"
        ...
        /// @brief General matrix-matrix product (single-precision) with explicit transpose flags.
        ///
        /// Single-precision variant of the double-precision overload. Semantics match the double version.
        ///
        /// @param Ta   Pointer to a char indicating whether matrix A is transposed ("N"/"T").
        /// @param Tb   Pointer to a char indicating whether matrix B is transposed ("N"/"T").
        /// @param m    Number of rows of op(A) and C.
        ...
        /// @brief General matrix-matrix product (single-precision) with defaults.
        ///
        /// Convenience overload for float data with default scalars and no transposition.
        ///
        /// @param m    Number of rows of op(A) and C.
        /// @param n    Number of columns of op(B) and C.
        /// @param k    Number of columns of op(A) / rows of op(B).
        ...
        /// @brief General matrix-matrix product (double-precision complex) with defaults.
        ///
        /// Convenience overload that assumes no transposition by default and common default
        /// scalars (alpha = 1.0, beta = 0.0) for complex-double data.
        ...
        ///
        /// Complex single-precision variant (wrapper to cgemm). Semantics match the complex-double version.
        void MatrixMatrixProduct(const char *Ta, const char *Tb, const size_t m, const size_t n, const size_t k,
                                 const complex_float alpha, const complex_float *a, const complex_float *b, const complex_float beta, complex_float *c);

        /// @brief General matrix-matrix product (single-precision complex) with defaults.
        ///
        /// Convenience overload for complex-float data with default scalars and no transposition.
        ...
        /// @brief Symmetric matrix-matrix product (double-precision) with explicit side.
        ///
        /// Performs C = alpha * A * B + beta * C or C = alpha * B * A + beta * C depending on `side`.
        /// A is assumed symmetric and stored in the convention used by the caller.
        ///
        ...
        /// @brief Symmetric matrix-matrix product (double-precision) with defaults.
        ///
        /// Convenience variant that defaults to left-side application (CblasLeft) and typical scalar values.
        ...
        /// @brief Symmetric matrix-matrix product (single-precision) with explicit side.
        ///
        /// Single-precision variant of the double-precision symmetric multiply. Semantics match the double version.
        ///
        /// @param side  Which side the symmetric matrix A appears on (CblasLeft or CblasRight).
        /// @param m     Number of rows of C.
        /// @param n     Number of columns of C.
        /// @param alpha Scaling factor for the product involving A (float precision).
        /// @param a     Pointer to symmetric matrix A data (float precision).
        /// @param b     Pointer to matrix B data (float precision).
        /// @param beta  Scaling factor for the existing contents of C (float precision).
        /// @param c     Pointer to output matrix C data (float precision).
        void SymMatrixMatrixProduct(const CBLAS_SIDE side, const size_t m, const size_t n, const float alpha, const float *a,
                                    const float *b, const float beta, float *c);

        /// @brief Symmetric matrix-matrix product (single-precision) with defaults.
        ///
        /// Convenience variant for float data which defaults to left-side application and standard scalars.
