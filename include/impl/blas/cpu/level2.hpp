...
namespace lahva
{
    namespace cpu
    {
        ...
        ///
        /// Allows calling the outer product wrapper with scalar types that differ
        /// from the vector element type (for example, passing double literals
        /// when T==float). Scalars are converted to `T` before forwarding.
        ///
        ...
        /// @brief Matrix-vector multiply wrapper (GEMV-like).
        ///
        /// Performs y := alpha * op(A) * x + beta * y where op(A) is A,
        /// A^T or A^H depending on `Ta` ("N","T","C"). This C++ template
        /// accepts Matrix_/Vector_ typed storage and forwards to the lower-level
        /// pointer-based implementation that calls the appropriate BLAS routine.
        ///
        /// @tparam T Element type for matrix and vectors (double, float, complex_double, complex_float).
        ...
        /// @param y     Output vector (destination / input-output).
        ...
        ///
        /// Allows calling the matrix-vector wrapper with scalar types that differ
        /// from the matrix element type (for example, passing double literals
        /// when T==float). Scalars are converted to `T` before forwarding.
        ///
        ...
        /// the matrix and vectors for the most common usage pattern, while still
        /// forwarding to the full MatrixVectorProduct(Ta,alpha,...) overload for flexibility.
        ///
        /// @tparam T Element type for matrix and vectors.
        /// @param a     Left-hand input matrix (Matrix_<T>).
        /// @param x     Input vector (Vector_<T>).
        /// @param y     Output vector (destination / input-output).
        /// @param Ta    Transpose option for A ("N","T","C"), default "N".
        ...
        /// @brief Simple convenience wrapper: y := alpha*A*x + beta*y with defaults.
        ///
        /// This inline overload uses default transpose="N", alpha=1 and beta=0 to
        /// support the common case y := A*x. It simply forwards to the full
        /// MatrixVectorProduct(Ta,alpha,...) overload for flexibility.
        ...
        /// The matrix `a` must follow the project's triangular storage convention;
        /// the implementation will use the appropriate BLAS `symv` routine.
        ///
        /// @param alpha Scaling factor applied to A*x.
        /// @param a     Symmetric matrix (Matrix_<double>).
        ...
        /// @brief  Symmetric matrix-vector multiply convenience overload with defaults (double precision).
        /// @param a     Symmetric matrix (Matrix_<double>).
        /// @param x     Input vector (Vector_<double>).
        /// @param y     Output vector (destination / input-output).
        ...
        ///
        /// Float variant of SymMatrixVectorProduct; forwards to the BLAS `ssymv` routine.
        ...
        /// @brief  Symmetric matrix-vector multiply convenience overload with defaults (single precision).
        /// @param a     Symmetric matrix (Matrix_<float>).
        ...
        /// @brief Symmetric matrix-vector multiply using lower-triangular storage (double).
        ///
        /// Variant that accepts a `LowTriMatrix_` representation (lower-triangular
        ...
        /// @brief  Symmetric matrix-vector multiply convenience overload with defaults (double precision) for LowTriMatrix_ storage.
        /// @param a     Lower-triangular matrix (LowTriMatrix_<double>).
        /// @param x     Input vector (Vector_<double>).
        /// @param y     Output vector (destination / input-output).
        /// @param alpha Scaling factor applied to A*x, default 1.0.
        /// @param beta  Scaling factor applied to existing contents of y, default 0.0.
        ...
        /// @brief Symmetric matrix-vector multiply using lower-triangular storage (float).
        ///
        /// Float variant specialized for `LowTriMatrix_` storage.
        void SymMatrixVectorProduct(const float alpha, const LowTriMatrix_<float> &a, const Vector_<float> &x,
                                    const size_t incx, const float beta, Vector_<float> &y, const size_t incy);

        /// @brief  Symmetric matrix-vector multiply convenience overload with defaults (single precision) for LowTriMatrix_ storage.
        /// @param a     Lower-triangular matrix (LowTriMatrix_<float>).
        /// @param x     Input vector (Vector_<float>).
        /// @param y     Output vector (destination / input-output).
        /// @param alpha Scaling factor applied to A*x, default 1.0.
        /// @param beta  Scaling factor applied to existing contents of y, default 0.0.
        ...
        /// @brief Lower-triangular matrix-vector product / triangular matrix-vector multiply (double).
        ///
        /// Performs x := op(T) * x where T is a lower-triangular matrix stored in
        /// `LowTriMatrix_`. The parameter `T` controls transpose ("N","T","C")
        /// and `unit` indicates whether the diagonal is assumed unit.
        ///
        /// @param T    Transpose option for the triangular matrix ("N","T","C").
        /// @param unit Whether the diagonal is unit (CblasUnit/CblasNonUnit).
        /// @param a    Lower-triangular matrix (LowTriMatrix_<double>).
        /// @param x    Vector to be multiplied (in-place output).
        /// @param incx Stride for x.
        ...
        /// @brief  Lower-triangular matrix-vector product convenience overload with defaults (double precision).
        /// @param a     Lower-triangular matrix (LowTriMatrix_<double>).
        /// @param x     Vector to be multiplied (in-place output).
        /// @param T     Transpose option for the triangular matrix ("N","T","C").
        /// @param unit  Whether the diagonal is unit (CblasUnit/CblasNonUnit).
        ...
        /// @brief Lower-triangular matrix-vector product / triangular matrix-vector multiply (float).
        ///
        ...
        /// @brief  Lower-triangular matrix-vector product convenience overload with defaults (single precision).
        /// @param a     Lower-triangular matrix (LowTriMatrix_<float>).
        /// @param x     Vector to be multiplied (in-place output).
        /// @param T     Transpose option for the triangular matrix ("N","T","C").
        /// @param unit  Whether the diagonal is unit (CblasUnit/CblasNonUnit).
        ...
        /// @brief Runtime-dispatch wrapper for OuterVectorProduct.
        ///
        /// Accepts a `CPURuntime` first argument to keep the API uniform with
        /// other backends; the runtime is ignored for CPU implementations and
        /// the call forwards to the primary overload.
        ...
        /// @brief Runtime-dispatch wrapper for LowTriMatrixVectorProduct.
        template <typename... Args>
        ...
        /// @brief Runtime-dispatch wrapper for MatrixVectorProduct.
        template <typename... Args>
        ...
        /// @brief Runtime-dispatch wrapper for SymMatrixVectorProduct.
        template <typename... Args>
